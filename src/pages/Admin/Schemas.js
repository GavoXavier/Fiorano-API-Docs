...
const API_URL = "http://localhost:5000/schemas"; // Backend API Endpoint
...
const Schemas = () => {
  ...
  const [schemas, setSchemas] = useState([]); // Stores all schemas

  // ✅ Fetch all schemas from MySQL
  useEffect(() => {
    axios.get(API_URL)
  ...
  // ✅ Handle form submission (Save Schema to MySQL)
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
            ...
            {formData[field].map((item, index) => (
              <div key={index} className="flex items-center gap-2 mb-2">
                ...
                <input
                  ...
                  placeholder="Type"
                  value={item.type || ''}
                  onChange={(e) => handleArrayFieldChange(field, index, "type", e.target.value)}
                  className="w-1/3 p-2 border rounded dark:bg-gray-700 dark:text-white"
                />
                <input
                  type="text"
                  ...
                  onChange={(e) => handleArrayFieldChange(field, index, "description", e.target.value)}
                  className="w-1/3 p-2 border rounded dark:bg-gray-700 dark:text-white"
                />
                <button type="button" onClick={() => removeArrayField(field, index)} className="text-red-500">
                  Remove
                </button>
              </div>
            ))}
            <button type="button" onClick={() => addArrayField(field)} className="text-blue-500">
